...
public class ConnectDatabase {
    ...
    public Livre getLivre(String auteur, String titre){
     ...
     public List<Livre> getLivresEmpruntes(){
        ...
        try {
            listLivres =new ArrayList<>();
            rs =   this.cnx.createStatement().executeQuery( "SELECT L.TITRE, L.AUTEUR, L.SORTIE, U.NOM, U.PRENOM, U.MAIL, U.ROLE FROM LIVRE as L INNER JOIN EMPRUNT ON L.ID = EMPRUNT.LIVRE, UTILISATEUR as U INNER JOIN EMPRUNT ON U.ID = EMPRUNT.UTILISATEUR GROUP BY L.TITRE,L.AUTEUR,L.SORTIE, U.NOM, U.PRENOM, U.MAIL, U.ROLE ");
           while(rs.next()){
               Livre l = new Livre();
               l.setAuteur(rs.getString("AUTEUR"));
               l.setTitre(rs.getString("TITRE"));
